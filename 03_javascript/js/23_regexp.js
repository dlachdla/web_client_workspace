 ...
 *  - 조건/반복/처리를 내부적으로 진행해서 간결하게 작성이 가능하다. 
 *  - 문법자체 숙지 어려운편.
 *  - 언어독립적.
 * 
 */
document.querySelector("#btn1").addEventListener('click', () => {
    ...
    console.log("a1b2c3d".split(re)); // ['a', 'b', 'c', 'd'] 숫자를 기준으로 자름

    // String#replace(re, newString): string
    console.log("abc-123".replace(re, '*')); // abc-*** 숫자에 해당하는 부분이 바뀜
    console.log("abc1def2ghi".replace(re, "<$&>")); // abc<1>def<2>ghi '$&'=는 대체문자
    ...
    console.log("xyzㅋㅋㅋ".search(re)); // -1
...
xxx`;
    ...
    area.innerHTML += `<p>${src2.replace(/x$/gi, '<mark>$&</mark>')}</p>`;
});
...
document.querySelector("#btn4").addEventListener('click', (e) => {
    ...
    console.log(re.test('a b')); // true 공백도 하나의 문자여서 true
...
/**
 * [] 한글자에 대한 값목록
 *  - Ascil code 기반으로 범위설정 가능
 *  - ^으로 시작하면 반전처리
 */
document.querySelector("#btn5").addEventListener('click', (e) => {
    const src = `ABCDEFGhijlmn 가나다라마바사ㅋㅋㅎ허ㅓㅏㅏ 1234567890 !@#$%^&*()\t\n`;
    const area = e.target.nextElementSibling;

    area.innerHTML = `<p>${src.replace(/[aj]/gi, '<mark>$&</mark>')}</p>`
    area.innerHTML += `<p>${src.replace(/[^aj]/gi, '<mark>$&</mark>')}</p>` // a,j가 아닌문자
    area.innerHTML += `<p>${src.replace(/^[aj]/gi, '<mark>$&</mark>')}</p>` // a나j로 시작(첫번째)하는문자

    area.innerHTML += `<p>${src.replace(/[123456789]/gi, '<mark>$&</mark>')}</p>`
    area.innerHTML += `<p>${src.replace(/[0-9]/gi, '<mark>$&</mark>')}</p>`
    area.innerHTML += `<p>${src.replace(/[abcdefghigklmnopqrstuvwxyz]/g, '<mark>$&</mark>')}</p>` //소문자 
    area.innerHTML += `<p>${src.replace(/[a-z]/g, '<mark>$&</mark>')}</p>`
    area.innerHTML += `<p>${src.replace(/[ABCDEFGHIJKLMNOPQRSTUVWXYZ]/g, '<mark>$&</mark>')}</p>` //대문자
    area.innerHTML += `<p>${src.replace(/[A-Z]/g, '<mark>$&</mark>')}</p>`
    ...
    area.innerHTML += `<p>${src.replace(/[ㄱ-ㅎ ㅏ-ㅣ 가-힣]/g, '<mark>$&</mark>')}</p>`

    // 유의사항
    // 반전을 의미하려면 ^캐럿은 맨첫째자리만 사용가능
    // /[A-Z^Q]/ X없음 (Q를 빼려면)
    // /[A-PR-Z]/ O이렇게해야함
    ...
    console.log(re.test('a ㅏㅏㅏㅏㅏ')); // true
...
/**
 * Escaping
 * - ^$|[]. 문법요소를 문자그대로 사용하려면 \를 이용해 escaaping처리 필수
 */
