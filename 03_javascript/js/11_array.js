...
function test1(){
    ...
    const arr2 = new Array(1, 2, 3); // new연산자로 생성

    console.log(arr1);
    console.log(arr2);
    console.log(arr1 == arr2); // false 요소가 같아도 다른배열이여서 
   ...
   const student = ['홍길동', [100, 98, 50], [90, 80, 90]];
   console.log((student[1][0] + student[2][0]) / 2); // 홍길동학생의 국어평균구하기
}
...
function test4(){
    ...
    console.log([...buttons]); // [[Prototype]] : Array로 바뀜
...
function test5(){
    ...
    console.log(vegitables); // 원본 변경됨

    // sort (mutable) : 오름차순정렬
    const nums = [3, 2, 1, 5, 4];
    console.log(nums.sort())
    console.log(nums); // 원본 변경
...
function test9(){
    ...
    const arr4 = arr3.map(function(n){
        return n.innerHTML;
    });
    console.log(arr4);
}
