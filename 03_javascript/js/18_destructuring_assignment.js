 ...
 *  - 배열은 인덱스에 따라, 객체는 속성명에 따라 변수에 대입처리
 */
...
const test3 = () => {
    ...
    // 변수명을 달리하는 경우
    const {a : num, b : season, c : bool, d : zzz = 'ㅎㅎㅎ'} = obj; // zzz가 없으면 ㅎㅎㅎ로 넣어주기
    console.log(num, season, bool, zzz); // 'Winter' true 'ㅎㅎㅎ'
...
const test4 = () => {
    ...
    // 홈페이지 : ...
    // 전화번호 : ...(대표) , ....
    // 지점 1 : 강남지원(서울시 강남구 역삼동)
    // 지점 2 : 종로지원(서울시 강남구 역삼동)
    // 지점 3 : 당산지원(서울시 강남구 역삼동)
};
...
const getAcademyInfo = () => ({
    ...
    branches : {
        '강남지원' : { addr : '서울시 강남구 역삼동' },
        '종로지원' : { addr : '서울시 종로구 탑골동' },
        '당산지원' : { addr : '서울시 영등포구 당산동' }
    }
});